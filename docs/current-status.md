...
- Schema target advanced to `0.17`; `0.16` added durable `role_permissions` assignments and `0.17` adds activity-dashboard performance indexes.
...
- Settings → General includes an OxyGen CMS Version & Update Readiness panel backed by `GET /api/system/version` and `GET /api/system/update-status`, showing current package/build metadata, GitHub latest-release/latest-tag update status, offline warning handling, guarded dry-run/confirmed update commands, ordered dry-run/backup/checkout/build/restart/schema readiness steps, and a newer-version notice/banner for SystemAdmin users. Guarded runner endpoints (`POST /api/system/update-runner/dry-run` and `/api/system/update-runner/update`) are implemented, disabled by default, and require explicit deployment config plus confirmation for real updates.
- Phase 1.5/Milestone 8 queue foundation is underway: BullMQ/Bull Board dependencies are installed, Redis is available in local/deployment Compose, queue env settings are documented, `GET /api/system/queues` exposes disabled/BullMQ queue counts for SystemAdmins, optional Bull Board mounts at `/admin/queues` when enabled, Settings → General shows a BullMQ / Redis Orchestration card with mode, Redis status, totals, named queue counts, a sanitized latest-jobs view backed by `GET /api/system/queue-jobs`, and a Bull Board link, and opt-in worker services/entrypoints are available through the Compose `workers` profile. The `instance-checks` worker processor safely dispatches saved-instance connectivity checks through the existing repository persistence path using ID-only payloads and now prevents overlapping same-instance checks inside a worker runtime; automatic startup/CRUD/import schedule reconciliation is wired behind the BullMQ opt-in gate with deterministic scheduler IDs, stale schedule cleanup, interval clamping, and jittered starts. Manual enqueue helpers and the first `database-maintenance` `purge-logs` queued job/processor path are available for follow-on UI actions.

Active next implementation focus: Phase 1.5/Milestone 8 — continue from the completed BullMQ runtime/native latest-jobs checkpoint into retry/backoff by failure class, guarded Analyze/Optimize database maintenance jobs, backup/restore job design with safety gates, Settings → Database queued-job visibility, and the fuller native Jobs dashboard filters/details; Milestone 7D UI action buttons and automatic schema-migration follow-through remain pending.
