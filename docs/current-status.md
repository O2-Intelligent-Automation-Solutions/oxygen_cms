...
- Phase 1.5/Milestone 8 queue foundation is underway: BullMQ/Bull Board dependencies are installed, Redis is available in local/deployment Compose, queue env settings are documented, `GET /api/system/queues` exposes disabled/BullMQ queue counts for SystemAdmins, optional Bull Board mounts at `/admin/queues` when enabled, Settings → General shows a BullMQ / Redis Orchestration card with mode, Redis status, totals, named queue counts, and Bull Board link, and opt-in worker services/entrypoints are available through the Compose `workers` profile. The `instance-checks` worker processor now safely dispatches saved-instance connectivity checks through the existing repository persistence path using ID-only payloads; opt-in schedule reconciliation and manual enqueue helpers define deterministic scheduler IDs, stale schedule cleanup, interval clamping, and jittered starts. Automatic startup/CRUD reconciliation wiring remains next.

Active next implementation focus: Phase 1.5/Milestone 8 — automatic queue startup/CRUD reconciliation and substantial frontend review surface; Milestone 7D UI action buttons and automatic schema-migration follow-through remain pending.
