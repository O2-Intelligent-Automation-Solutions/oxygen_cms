...
- Phase 1.5/Milestone 8 queue foundation is underway: BullMQ/Bull Board dependencies are installed, Redis is available in local/deployment Compose, queue env settings are documented, `GET /api/system/queues` exposes disabled/BullMQ queue counts for SystemAdmins, optional Bull Board mounts at `/admin/queues` when enabled, Settings → General shows a BullMQ / Redis Orchestration card with mode, Redis status, totals, named queue counts, and Bull Board link, and opt-in worker services/entrypoints are available through the Compose `workers` profile. The `instance-checks` worker processor safely dispatches saved-instance connectivity checks through the existing repository persistence path using ID-only payloads; automatic startup/CRUD/import schedule reconciliation is wired behind the BullMQ opt-in gate with deterministic scheduler IDs, stale schedule cleanup, interval clamping, and jittered starts. Manual enqueue helpers and the first `database-maintenance` `purge-logs` queued job/processor path are available for follow-on UI actions.

Active next implementation focus: Phase 1.5/Milestone 8 — substantial frontend review surface and database/system maintenance queue jobs; Milestone 7D UI action buttons and automatic schema-migration follow-through remain pending.
