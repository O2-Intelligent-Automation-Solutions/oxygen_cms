...
- Settings → General includes an OxyGen CMS Version panel backed by `GET /api/system/version`, showing current package/build metadata, GitHub latest-release/latest-tag update status, offline warning handling, and a newer-version notice/banner for SystemAdmin users. Milestone 7D now also exposes `GET /api/system/update-status` for the future non-technical update flow; the initial read-only status reports guarded command availability and ordered update steps while live runner execution remains next.
