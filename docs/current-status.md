...
- Phase 1.5/Milestone 8A queue foundation is underway: BullMQ/Bull Board dependencies are installed, Redis is available in local/deployment Compose, queue env settings are documented, `GET /api/system/queues` exposes disabled/BullMQ queue counts for SystemAdmins, optional Bull Board mounts at `/admin/queues` when enabled, Settings → General shows a BullMQ / Redis Orchestration card with mode, Redis status, totals, named queue counts, and Bull Board link, and opt-in worker services/entrypoints are available through the Compose `workers` profile. Per-instance scheduled jobs and real job processors remain next.
