...
- `GET|PUT /api/app-settings/queue-schedules` persists Settings → General queue maintenance schedule cards. The UI displays whole-day cadence with `Every [n] day(s)` and a one-day minimum; the API stores `everySeconds`.
