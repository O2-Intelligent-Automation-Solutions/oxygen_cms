...
- `GET /api/system/update-status` returns the Milestone 7D read-only update orchestration status contract: guarded command availability, confirmation requirement, ordered dry-run/backup/checkout/build/restart/schema steps, and placeholders for future live runner history/error details.
- `GET /api/system/queues` returns the Phase 1.5/Milestone 8A queue orchestration status contract for SystemAdmins: disabled/BullMQ mode, Redis configuration/connectivity, and waiting/active/delayed/failed/completed counts for `instance-checks`, `database-maintenance`, and `system-maintenance`. Optional Bull Board mounts at the configured admin path behind the same CMS auth/RBAC guard when enabled.
