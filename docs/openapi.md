...
- `GET /api/system/queues` returns the Phase 1.5/Milestone 8A queue orchestration status contract for users with `jobs.view`: disabled/BullMQ mode, Redis configuration/connectivity, Bull Board enabled/path metadata, and waiting/active/delayed/retained-failed/completed counts for `instance-checks`, `database-maintenance`, and `system-maintenance`. Optional Bull Board mounts at the configured admin path behind the same CMS auth/RBAC guard when enabled; Settings uses native queue visibility as the primary review UI.
- `GET /api/system/queue-jobs` returns a Phase 1.5 sanitized native queue visibility contract for users with `jobs.view`: active/ready/scheduled/retained latest jobs from the named queues with queue/name/state/attempt/timestamp/failure summary, safe timing/resource metrics (`phase`, `ageSeconds`, `waitSeconds`, `durationMs`, `attemptCost`), sanitized database-maintenance result summaries (table/warning/artifact counts and short text), and safe public metadata (`task`, `source`, `instanceId`, `requestedBy`) only. Raw BullMQ payloads, result rows, artifact paths, and secrets are not returned.
- `POST /api/system/queue-jobs/{key}/run-now`, `/pause`, and `/resume` require `jobs.manage` and provide one-off execution and recurring schedule state controls for maintenance scheduler keys and `instance-check:{instanceId}` rows. Run Now does not change the recurring enabled/paused state; guarded database maintenance keys include Analyze Tables, Optimize Tables, and Backup Database, with backup artifact creation still gated by worker deployment config.
- `GET|PUT /api/app-settings/queue-schedules` persists Settings → General queue maintenance schedule cards. The UI displays whole-day cadence with `Every [n] day(s)` and a one-day minimum. The API now normalizes interval schedules to `schedule: { type: 'interval', everySeconds }` while preserving legacy `everySeconds` compatibility; CRON/anchored schedules are designed but not accepted until the next runtime/UI slice.
...
- SSL certificate warning settings:
  - `GET /api/app-settings/ssl-certificate-warning`
  - `PUT /api/app-settings/ssl-certificate-warning`
  - The global `daysBeforeExpiration` threshold controls when otherwise valid HTTPS certificates display as `Expiring Soon`; expired certificates always display as `Expired` first.
- License expiration warning settings:
  - `GET /api/app-settings/license-expiration-warning`
  - `PUT /api/app-settings/license-expiration-warning`
  - The global `daysBeforeExpiration` threshold controls when otherwise valid OxyGen licenses display as `Expiring Soon`; expired licenses always display as `Expired` first and threshold warnings surface as real Dashboard/Issue Types warnings.
