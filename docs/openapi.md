...
- `GET /api/system/queue-jobs` returns a Phase 1.5 sanitized native queue visibility contract for SystemAdmins: active/ready/scheduled/retained latest jobs from the named queues with queue/name/state/attempt/timestamp/failure summary and safe public metadata (`task`, `source`, `instanceId`, `requestedBy`) only. Raw BullMQ payloads and secrets are not returned.
