...
- `POST /api/system/queue-jobs/{key}/run-now`, `/pause`, and `/resume` provide one-off execution and recurring schedule state controls for maintenance scheduler keys and `instance-check:{instanceId}` rows. Run Now does not change the recurring enabled/paused state.
