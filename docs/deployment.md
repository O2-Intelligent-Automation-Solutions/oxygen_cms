...
- Tagged-update validation remains next. Local deployment-helper validation has passed for `scripts/deploy.sh init`, `scripts/deploy.sh check`, and `scripts/deploy.sh update --dry-run main`; clean isolated Compose deployment and backup/restore smokes also pass with `/api/health`, the SPA root, `scripts/deploy.sh backup`, and guarded `CONFIRM_RESTORE=YES scripts/deploy.sh restore-db`. The host-side guarded `scripts/deploy.sh update` command, backend execution/status tracking, Settings UI action buttons, automatic post-restart schema migration follow-through, and in-CMS recovery guidance are present.
