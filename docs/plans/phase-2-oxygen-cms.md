...
- [ ] Complete trigger/workflow error MVP through CMS-native Processing Errors parity: source/build-validated typed CMS Workflow Trigger, Workflow Event, and Service Event grids now use classic server paging; selecting a workflow event with a `ServiceIdentifier` lazy-loads parent service-event rows and child rows expand on demand; the current slice also includes a read-only Event Details panel plus confirmed row actions for trigger cancel, workflow-event recovery/cancel, and service-event restore behind granular permissions. Remaining closeout is file downloads, EMM/WHE message viewers, bulk actions with scoped previews/guardrails, live OxyGen/browser validation, and final completion/recovery tracking.
